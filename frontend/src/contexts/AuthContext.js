...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    if (!API) return { error: "Backend bağlantısı yapılandırılmamış." };
    try {
  ...
  const verify2FALogin = async (code, tempToken) => {
    ...
    try {
      const { data } = await axios.post(`${API}/auth/verify-2fa-login`, { code }, {
        withCredentials: true, headers: { Authorization: `Bearer ${tempToken}` }
      });
      setUser(data);
      return { success: true };
    } catch (e) {
  ...
  const logout = async () => {
    try { if (API) await axios.post(`${API}/auth/logout`, {}, { withCredentials: true }); } catch {}
    setUser(false);
  };
